...
- `src/Interaction.cs`：8 个互动动作的状态、时长、帧映射和阻塞逻辑。
...
已验证 8 个互动动作的按钮映射、素材解码、被摸唤醒、移动暂停、阻塞暂停和生命周期；验证日志与互动动作联系图位于 `output/`。
...
第三版在基础动作和本地固定文案气泡上加入 8 个本地互动动作，不包含自由输入的 AI 对话、语音、联网功能或开机启动。猫咪为参考风格重新生成的角色素材，并非逐像素还原原图。
