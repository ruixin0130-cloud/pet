...
自由活动时，走路和跑步会消耗精力，坐下、趴下和睡觉会恢复精力。精力较低时玉子会自动进入睡眠，恢复后再醒来继续活动；面板中的精力条会实时显示当前状态。

摸摸玉子后会停留约 8 秒陪伴你，期间暂停自主动作和随机气泡，避免刚回应完就被下一次自动切换打断。手动选择其他动作或切换模式会立即结束这段陪伴。
...
- 互动动作带轻微的上下弹动，素材中的问号、毛线球和手掌装饰保留参考图的轻盈感觉。
...
- 程序和系统托盘会显示玉子的灰白猫咪图标，图标包含 16 至 256 像素的多个尺寸，适合任务栏、桌面和资源管理器显示。
- 重复启动会唤出已有动作面板，不会出现多只猫咪。
- 要带到另一台 Windows 电脑，请复制 `玉子桌宠.exe` 和同级的 `content` 文件夹；删除 `content` 也能启动，只会使用默认内容。
...
- `src/PetProfile.cs`：内容配置包读取、校验和默认值回退。
- `src/Panel.xaml`：原生动作面板。
- `content/tamago-profile.json`：可编辑的角色、动作、互动、对话与精力配置。
- `assets/tamago-sprites.png`：内置 imagegen 生成的 16 帧透明动作图。
- `assets/tamago-interactions.png`：内置 imagegen 生成的互动动作图集，当前启用其中 5 个姿势。
...
已验证动作按钮、移动与边缘转向、跳跃回落、拖拽暂停逻辑、精力消耗与自动休息唤醒、非法时间与设置恢复、16 帧解码、大小/速度调节、置顶开关、找回位置、面板收起与恢复。验证日志与渲染预览位于 `output/`。
