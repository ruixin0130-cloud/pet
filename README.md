...
- `src/Interaction.cs`：8 个互动动作的状态、时长、帧映射、阻塞逻辑与自主互动调度。
- `src/App.cs`：透明桌面窗口、面板绑定、拖拽、托盘、基础动画、互动动作与气泡渲染。
...
已验证 8 个互动动作的按钮映射、素材解码、被摸唤醒、移动暂停、阻塞暂停和生命周期；已验证自主互动的首次等待、随机间隔、忙碌暂缓、开关和连续防重复；验证日志与互动动作联系图位于 `output/`。
...
第四版在基础动作、随机气泡和 8 个本地互动动作上加入自主互动调度，不包含自由输入的 AI 对话、语音、联网功能或开机启动。猫咪为参考风格重新生成的角色素材，并非逐像素还原原图。
